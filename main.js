...
function createConts()
{
    ...
    conts.stock = new CardContainer(1, `calc(var(--card-size) * .2)`, 'calc(var(--card-size) * .1)')
    conts.waste = new CardContainer(3, `calc(var(--card-size) * ${1.1 + .2})`, 'calc(var(--card-size) * .1)')
    conts.foundation = []
    for(let i = 0; i < 4; ++i)
    {
        conts.foundation.push(new CardContainer(1, `calc(var(--card-size) * ${(3 + i) * 1.1 + .2})`, 'calc(var(--card-size) * .1)'))
    ...
    for(let i = 0; i < 7; ++i)
    {
        conts.tableau.push(new CardContainer(2, `calc(var(--card-size) * ${i * 1.1 + .2})`, 'calc(var(--card-size) * 1.7)'))
...
function undo()
{
    ...
    step.forEach((substep, i) => {
        ...
        if(substep.from ===  conts.stock)
            substep.card.hide()
        if(substep.from ===  conts.waste)
            substep.card.reveal()

        if(i === 0 && substep.from.type === CardContainer.TYPE.TABLEAU && substep.from.cards.length > 1)
        {
            const lastBefore = substep.from.cards[substep.from.cards.length - 2]
            const currentCard = substep.card

            if(lastBefore.color === currentCard.color || lastBefore.number !== currentCard.number + 1)
                lastBefore.hide()
        }
    })
}
...
function timer()
{
    ...
    const timeStr = `${~~(timeElapsed / 60)}:${timeElapsed % 60 >= 10 ? '' : '0'}${timeElapsed % 60}`
...
{
    if(!(bypass || confirm('Are you sure you want to restart the game?')))
        return
...
function setupGameOverAnimation()
{
    ...
    canvas.height = Math.min(document.getElementById('game-area').offsetHeight, document.getElementById('game-area').offsetWidth * .6)
    ...
    function resize()
    {
        ...
        if(desktopMode)
            cardSheet.src = './assets/desktop/allcards.png'
    }
    ...
    {
        ctx.drawImage(cardSheet, number * card_texture_w, sign * card_texture_h, card_texture_w, card_texture_h, x, y, card_w, card_h)
    }
...
function fullAutoplayStep()
{
    ...
    ;[...conts.tableau, conts.waste].forEach(cont => {
        ...
        cont.cards.forEach(card => {
            ...
            if(!card.upsideDown)
            {
                const goal = cont.type === CardContainer.TYPE.TABLEAU ? card.getNextStep() : card.container.lastCard.getNextStep()

                if(goal)
                {
                    if(cont.type === CardContainer.TYPE.TABLEAU && goal.type === CardContainer.TYPE.TABLEAU && !((card.container.getCardBefore(card) && card.container.getCardBefore(card).upsideDown) || (!card.container.getCardBefore(card) && goal.cards.length > 0)))
                    {
                        return
                    }
...
async function autoplayStep(goal, movedCard)
{
    ...
    ;[...conts.tableau, conts.waste].forEach(cont => {
        ...
        if(cont.lastCard && !cont.lastCard.bypassAutoplay && cont.lastCard !== movedCard && cont.lastCard.getNextStep() === goal && !positiveOutcome) {
